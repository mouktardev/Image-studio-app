...
// Model configuration for BRIA RMBG-1.4
...
fn check_model_downloaded() -> Result<(PathBuf, i64), String> {
...
fn run_bg_removal_inference(
    ...
    // Set up the ONNX Runtime environment (CPU only for maximum compatibility)
    let environment = Arc::new(
        Environment::builder()
            .with_name("background-removal")
    ...
    // Create input array with shape [1, 3, 1024, 1024]
    let mut input_array = Array::zeros((1, 3, 1024, 1024));

    for y in 0..1024 {
        for x in 0..1024 {
            let pixel = resized_image.get_pixel(x, y);
            input_array[[0, 0, y as usize, x as usize]] = pixel[0] as f32 / 255.0; // R
            input_array[[0, 1, y as usize, x as usize]] = pixel[1] as f32 / 255.0; // G
            input_array[[0, 2, y as usize, x as usize]] = pixel[2] as f32 / 255.0; // B
        }
    }
    ...
    // Create mask image from the output
